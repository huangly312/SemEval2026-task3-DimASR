...
out_put_file_name_map = {
    'res_eng': "pred_eng_restaurant",
    'lap_eng': "pred_eng_laptop",
    'lap_zho': "pred_zho_laptop",
    'res_zho': "pred_zho_restaurant",
}
...
def parser_getting():
    ...
    parser.add_argument('--exclude_NULL', type=bool, default=False)
...
def inference(args, model, tokenize, batch_generator, beta, logger, gpu, max_len, category_mapping):
    ...
    out_put_file_task2_name = args.output_path + "subtask_2/" + f"subtask_{args.task}_"+out_put_file_name_map[args.domain + '_' + args.language]+"_exclude_NULL_"+str(args.exclude_NULL)+".jsonl"
    ...
    if args.task == 3:
        out_put_file_task3_name = args.output_path + "subtask_3/" + f"subtask_{args.task}_"+out_put_file_name_map[args.domain + '_' + args.language]+"_exclude_NULL_"+str(args.exclude_NULL)+".jsonl"
...
def load_train_data_multilingual(args):
    ...
    for i, data in enumerate(all_data):
        ...
        for quad in quadruplets:
            # Skip quadruplets with NULL aspect if exclude_NULL is True
            if args.exclude_NULL and (quad['Aspect'] == 'NULL' or not quad['Aspect'] or quad['Aspect'].strip() == ''):
                continue
        ...
        # Skip data if all quadruplets are filtered out (quintuplets is empty)
        if len(quintuplets) == 0:
            continue
